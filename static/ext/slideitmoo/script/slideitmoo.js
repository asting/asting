...
var SlideItMoo = new Class({
    ...
    /* slides elements */
    slide_delta: function(delta) {
        this.direction = delta > 0 ? 1 : -1;
        this.delta = Math.abs(delta);
        this.endingElem = (this.currentElement + delta) % this.totalElements;
        if (this.endingElem < 0) this.endingElem += this.totalElements;
        this.myFx.options.duration = (this.options.duration / this.delta).toInt();
        this.slide(this.direction);
    },
    slide2: function(index) {
        this.endingElem = (index + this.totalElements) % this.totalElements;
        var f,b, delta;
        for(var i=1;i<this.totalElements;i++){
           if (Math.abs(this.currentElement + i) % this.totalElements == this.endingElem) delta = i;
           if (Math.abs(this.currentElement - i + this.totalElements) % this.totalElements == this.endingElem) delta = -1*i;
           if (delta) {
               this.direction = delta > 0 ? 1 : -1;
        ...
        if (delta) {
            this.myFx.options.duration = (this.options.duration / delta).toInt();
            this.slide(this.direction);
        }
    },
    slide: function(direction) {
        ...
        this.myFx.start(fxDist).chain(function() {
            this.rearange(true);
            if (this.options.elemsSlide || (this.endingElem != null && this.endingElem !== this.currentElement)) {
        ...
        if (this.endingElem == currentIndex) this.endingElem = null;
        if (this.endingElem == null) {
            this.myFx.options.duration = this.options.duration;
